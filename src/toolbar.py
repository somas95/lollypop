...
#######################
    ...
    def _pop_infobox(self, widget, event):
        if Objects.player.current_track.id is not None and\
           Objects.player.current_track.id != Navigation.RADIOS:
            if event.button == 1:
    ...
    def _on_current_changed(self, player):
        ...
        if player.current_track.id is None or player.current_track.id == Navigation.RADIOS:
            self._progress.set_sensitive(False)
        ...
        if player.current_track.id == Navigation.RADIOS:
    ...
    def _on_status_changed(self, player):
        ...
        if is_playing:
            ...
            if player.current_track.id == Navigation.RADIOS and self._timeout:
